...
test.dbwriter <- function() {
	...
	print(loadSpatialFunctions(con));
        ...
        stopifnot(identical(dbExistsTable(con,"VANNES"),TRUE))
        sq <- dbSendQuery(con,"INSERT INTO VANNES VALUES('POLYGON ((100 300, 210 300, 210 200, 100 200, 100 300))'::GEOMETRY, 1)"); 
        selectedTable = dbGetQuery(con,"SELECT ST_AREA(the_geom) as area FROM VANNES");
        dbWriteTable(con, "vannes_area", selectedTable, overwrite=TRUE);
        stopifnot(identical(as.numeric(dbGetQuery(con,"SELECT ST_AREA(the_geom) as area FROM vannes_area")[[1]]),11000))
}
